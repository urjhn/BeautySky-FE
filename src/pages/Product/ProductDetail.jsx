...
// import { useCart } from "../../context/CartContext";
// import { useDataContext } from "../../context/DataContext";
// import reviewApi from "../../api/reviewApi"; // Import API đánh giá
// import Navbar from "../../components/Navbar/Navbar";
// import Footer from "../../components/Footer/Footer";
// import { formatCurrency } from "../../utils/formatCurrency";
...
//     try {
//       const response = await reviewApi.getReviewsByProductId(id);
...
//         if (foundProduct) {
//           setProduct(foundProduct);
//           await fetchReviews(); // Lấy đánh giá từ API
//         }
//       } catch (error) {
//         console.error("Lỗi khi tải chi tiết sản phẩm:", error);
//       } finally {
...
//       await fetchReviews(); // Cập nhật lại danh sách đánh giá
//       setNewRating(0);
//       setNewComment("");
//       setSelectedImage(null);
//     } catch (error) {
...
//             <p className="text-lg text-gray-600">{product.description}</p>
//             <p className="text-md text-gray-500">Loại da: {product.skinType?.skinTypeName || "Không xác định"}</p>
//             <p className="text-md text-gray-500">Loại sản phẩm: {product.category?.categoryName || "Không xác định"}</p>
//             <p className="text-3xl font-bold text-gray-500">{formatCurrency(product.price)}</p>

//             <div className="mt-6 flex space-x-4">
//               <button
//                 className={`w-full py-4 rounded-2xl font-semibold transition-all flex items-center justify-center gap-2 shadow-md ${
//                   product.quantity === 0 ? "bg-gray-400 text-gray-800 cursor-not-allowed" : "bg-blue-500 text-white hover:bg-blue-600"
//                 }`}
...
//         {/* Phần đánh giá */}
//         <div className="w-full max-w-6xl mt-10 bg-white p-6 rounded-3xl shadow-lg">
//           <h2 className="text-2xl font-bold text-gray-900 mb-4">Đánh giá từ người dùng</h2>
...
//                   </div>
//                   <p className="mt-2 text-gray-700">{review.comment}</p>
//                   {review.image && <img src={review.image} alt="Review" className="mt-2 w-32 h-32 object-cover rounded-lg" />}
//                 </div>
//               ))}
//             </div>
//           )}
...
//                 <FaStar
//                   key={i}
//                   className={`text-xl cursor-pointer ${i < newRating ? "text-yellow-400" : "text-gray-300"}`}
//                   onClick={() => setNewRating(i + 1)}
//                 />
//               ))}
//             </div>
...
//               onChange={(e) => setNewComment(e.target.value)}
//             />
//             <input type="file" className="mt-2" onChange={(e) => setSelectedImage(e.target.files[0])} />
//             <button className="mt-4 bg-blue-500 text-white px-6 py-2 rounded-lg hover:bg-blue-600" onClick={handleReviewSubmit}>
//               Gửi đánh giá
//             </button>
//           </div>
//         </div>
